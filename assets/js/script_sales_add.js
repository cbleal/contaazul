		...
		if (datatype != '') {			
			$.ajax({
				...
				success:function(json) {
					...
					for (var i in json) {
						html += '<div class="si"><a href="javascript:;" onclick="addProd(this)" data-id="'+json[i].id+'" data-price="'+json[i].price+'" data-name="'+json[i].name+'">'+json[i].name+' R$ '+json[i].price+'</div>';
					}
